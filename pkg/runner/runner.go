...
func (r *Runner) RunEnumeration() (err error, rets map[string][]int) {
	defer r.Close()
...
func (r *Runner) HandleResult() (err error, rets map[string][]int) {
	if r.scanner.ScanResults.IpPorts != nil {
		rets := make(map[string][]int)
		for s, m := range r.scanner.ScanResults.IpPorts {
			//fmt.Printf("%v\n", s)
			var ports []int
			for i, _ := range m {
				//fmt.Printf("%v\n", i)
